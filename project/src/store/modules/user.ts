...
import { userInfoResponseData,loginFormatData,loginResponseData } from "@/api/user/type";
//引入路由（常量路由）
...
export const useUserStore = defineStore('User', () => {
    ...
    async function userLogin(data: loginFormatData) {
        let res:loginResponseData  = await reqLogin(data)
        if (res.code === 200) {
            token.value = res.data as string //放到pinia当中
            return 'ok'
        } else {
    ...
    const userInfo = async () => {
        ...
        let res:userInfoResponseData = await reqUserInfo() //请求的时候记得携带token
        if (res.code === 200) {
